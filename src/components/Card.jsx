import React, { useState } from "react";

import Card from '@mui/material/Card';
import CardActions from '@mui/material/CardActions';
import CardContent from '@mui/material/CardContent';
import CardMedia from '@mui/material/CardMedia';
import Button from '@mui/material/Button';
import Typography from '@mui/material/Typography';


export default function CardComponent(props) {
    // const [hotelData,setHotelData]=useState(null);
    console.log(props);
    return (
        <Card sx={{ maxWidth: 345 }}>
            <div style={{ height: "25px", width: "20px", position: "absolute", display: "flex", alignItems: "center", justifyContent: "center", right: "35px", borderTopLeftRadius: "5px" }} onClick={() => props.cb(props.value.id)}>
                { props.value.favorite? <svg width="18" height="17" viewBox="0 0 18 17" fill="none" xmlns="http://www.w3.org/2000/svg" style={{backgroundColor: "red"}}>

<path d="M7.925 15.8746L6.75 14.8246C4.96667 13.2079 3.39567 11.6369 2.037 10.1116C0.679 8.58694 0 6.99961 0 5.34961C0 4.08294 0.433334 3.01628 1.3 2.14961C2.16667 1.28294 3.23333 0.849609 4.5 0.849609C5.23333 0.849609 5.996 1.03294 6.788 1.39961C7.57933 1.76628 8.31667 2.46628 9 3.49961C9.68333 2.46628 10.421 1.76628 11.213 1.39961C12.0043 1.03294 12.7667 0.849609 13.5 0.849609C14.7667 0.849609 15.8333 1.28294 16.7 2.14961C17.5667 3.01628 18 4.08294 18 5.34961C18 7.03294 17.2917 8.65361 15.875 10.2116C14.4583 11.7703 12.9083 13.3079 11.225 14.8246L10.075 15.8746C9.775 16.1413 9.41667 16.2786 9 16.2866C8.58333 16.2953 8.225 16.1579 7.925 15.8746ZM8.5 4.52461C7.91667 3.55794 7.3 2.87028 6.65 2.46161C6 2.05361 5.28333 1.84961 4.5 1.84961C3.5 1.84961 2.66667 2.18294 2 2.84961C1.33333 3.51628 1 4.34961 1 5.34961C1 6.08294 1.20833 6.83694 1.625 7.61161C2.04167 8.38694 2.604 9.18294 3.312 9.99961C4.02067 10.8163 4.82933 11.6496 5.738 12.4996C6.646 13.3496 7.6 14.2246 8.6 15.1246C8.71667 15.2413 8.85 15.2996 9 15.2996C9.15 15.2996 9.28333 15.2413 9.4 15.1246C10.4 14.2246 11.354 13.3496 12.262 12.4996C13.1707 11.6496 13.9793 10.8163 14.688 9.99961C15.396 9.18294 15.9583 8.38694 16.375 7.61161C16.7917 6.83694 17 6.08294 17 5.34961C17 4.34961 16.6667 3.51628 16 2.84961C15.3333 2.18294 14.5 1.84961 13.5 1.84961C12.7167 1.84961 12 2.05361 11.35 2.46161C10.7 2.87028 10.0833 3.55794 9.5 4.52461C9.43333 4.60794 9.35833 4.67461 9.275 4.72461C9.19167 4.77461 9.1 4.79961 9 4.79961C8.9 4.79961 8.80833 4.77461 8.725 4.72461C8.64167 4.67461 8.56667 4.60794 8.5 4.52461Z" fill="#1A1A1A" />

</svg>:
<svg width="18" height="17" viewBox="0 0 18 17" fill="none" xmlns="http://www.w3.org/2000/svg" style={{backgroundColor: "white"}}>

                    <path d="M7.925 15.8746L6.75 14.8246C4.96667 13.2079 3.39567 11.6369 2.037 10.1116C0.679 8.58694 0 6.99961 0 5.34961C0 4.08294 0.433334 3.01628 1.3 2.14961C2.16667 1.28294 3.23333 0.849609 4.5 0.849609C5.23333 0.849609 5.996 1.03294 6.788 1.39961C7.57933 1.76628 8.31667 2.46628 9 3.49961C9.68333 2.46628 10.421 1.76628 11.213 1.39961C12.0043 1.03294 12.7667 0.849609 13.5 0.849609C14.7667 0.849609 15.8333 1.28294 16.7 2.14961C17.5667 3.01628 18 4.08294 18 5.34961C18 7.03294 17.2917 8.65361 15.875 10.2116C14.4583 11.7703 12.9083 13.3079 11.225 14.8246L10.075 15.8746C9.775 16.1413 9.41667 16.2786 9 16.2866C8.58333 16.2953 8.225 16.1579 7.925 15.8746ZM8.5 4.52461C7.91667 3.55794 7.3 2.87028 6.65 2.46161C6 2.05361 5.28333 1.84961 4.5 1.84961C3.5 1.84961 2.66667 2.18294 2 2.84961C1.33333 3.51628 1 4.34961 1 5.34961C1 6.08294 1.20833 6.83694 1.625 7.61161C2.04167 8.38694 2.604 9.18294 3.312 9.99961C4.02067 10.8163 4.82933 11.6496 5.738 12.4996C6.646 13.3496 7.6 14.2246 8.6 15.1246C8.71667 15.2413 8.85 15.2996 9 15.2996C9.15 15.2996 9.28333 15.2413 9.4 15.1246C10.4 14.2246 11.354 13.3496 12.262 12.4996C13.1707 11.6496 13.9793 10.8163 14.688 9.99961C15.396 9.18294 15.9583 8.38694 16.375 7.61161C16.7917 6.83694 17 6.08294 17 5.34961C17 4.34961 16.6667 3.51628 16 2.84961C15.3333 2.18294 14.5 1.84961 13.5 1.84961C12.7167 1.84961 12 2.05361 11.35 2.46161C10.7 2.87028 10.0833 3.55794 9.5 4.52461C9.43333 4.60794 9.35833 4.67461 9.275 4.72461C9.19167 4.77461 9.1 4.79961 9 4.79961C8.9 4.79961 8.80833 4.77461 8.725 4.72461C8.64167 4.67461 8.56667 4.60794 8.5 4.52461Z" fill="#1A1A1A" />

                </svg>}
                
            </div>
            <CardMedia
                sx={{ height: 140 }}
                image={props.value.imageSrc}
                title="green iguana"
                style={{color:"red"}}
            />
            <CardContent>
                <Typography gutterBottom variant="h5" component="div" style={{ color: "#1A1A1A", fontWeight: "600" }}>
                    {props.value.name}
                </Typography>
                <Typography gutterBottom variant="h5" component="div" style={{ color: "#1A1A1A", fontWeight: "500" }}>
                    <span>&#8377;{props.value.price.toLocaleString()}</span>
                </Typography>
                <Typography gutterBottom variant="h5" component="div">
                    {props.value.freeBreakfast && <span style={{ color: "#0FA670" }}>Free breakfast</span>}
                </Typography>

            </CardContent>

        </Card>
    );
}